...
import { FarmacieInVenditaSb } from "@/components/FarmacieInVenditaSb";
...
export const FarmacieInVenditaLista = ({ regione, page }) => {
  ...
  const DatiJson = filteredData.slice(start, end);
            ...
            <div className="d-flex gap-2 mt-4 flex-wrap">
              {Array.from({ length: totalPages }, (_, i) => {
                ...
                const url =
                  pageNum === 1
                    ? `/farmacie/${regioneKey}`
                    : `/farmacie/${regioneKey}/${pageNum}`;

                return (
                  <Link
                    key={pageNum}
                    href={url}
                    className={`btn btn-sm ${pageNum === currentPage
                        ? "btn-dark"
                        : "btn-outline-dark"
                      }`}
                  >
